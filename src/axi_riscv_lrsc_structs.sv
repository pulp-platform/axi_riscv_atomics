...
module axi_riscv_lrsc_structs #(
    ...
    always_comb begin
        ...
        mst_req_o.ar.lock = 1'b0;
    end
    assign slv_r.data       = mst_resp_i.r.data;
    assign slv_r.last       = mst_resp_i.r.last;
    assign slv_r.id         = mst_resp_i.r.id;
    assign slv_r.user       = mst_resp_i.r.user;
    ...
    always_comb begin
        ...
        rifq_oup_pop    = 1'b0;
        rifq_oup_req    = 1'b0;
        if (mst_resp_i.r_valid && slv_r_ready) begin
            rifq_oup_id     = mst_resp_i.r.id;
            rifq_oup_pop    = mst_resp_i.r.last;
            rifq_oup_req    = 1'b1;
            if (rifq_oup_gnt) begin
                mst_req_o.r_ready = 1'b1;
                // For ACE, forward RRESP[3:2] as-is and act only on RRESP[1:0]
                // For AXI, RRESP[3:2] are absent, and RRESP[1:0] contain the full response code
                slv_r.resp = mst_resp_i.r.resp;
                if (mst_resp_i.r.resp[1] == 1'b0) begin
                    slv_r.resp[1:0] = {1'b0, rifq_oup_data.excl};
                end
                slv_r_valid = 1'b1;
            end
        end
    ...
    always @(posedge clk_i) begin
        if (~rst_ni) begin
            if (rifq_oup_req && rifq_oup_gnt) begin
                assert (rifq_oup_data_valid) else $error("Unexpected R with ID %0x!", mst_resp_i.r.id);
            end
        end
    end
// pragma translate_on
        ...
        case (ar_state_q)
            ...
            AR_IDLE: begin
                if (slv_req_i.ar_valid) begin
                    ar_push_addr = slv_req_i.ar.addr[AXI_ADDR_WIDTH-1:AXI_ADDR_LSB];
                    ar_push_excl = (slv_req_i.ar.addr >= ADDR_BEGIN && slv_req_i.ar.addr <= ADDR_END &&
                            slv_req_i.ar.lock && slv_req_i.ar.len == 8'h00);
                    if (ar_push_excl) begin
                        ar_wifq_exists_inp.data.addr = slv_req_i.ar.addr[AXI_ADDR_WIDTH-1:AXI_ADDR_LSB];
            ...
            AR_WAIT: begin
                ...
                if (mst_resp_i.ar_ready && mst_req_o.ar_valid) begin
                    ar_state_d = AR_IDLE;
                end
            end
    ...
    id_queue #(
        ...
        .clk_i              (clk_i),
        .rst_ni             (rst_ni),
        .inp_id_i           (mst_req_o.aw.id),
        .inp_data_i         ({mst_req_o.aw.addr[AXI_ADDR_WIDTH-1:AXI_ADDR_LSB], slv_req_i.aw.lock}),
...
// pragma translate_on
    ...
    always_comb begin
        ...
        aw_res_len = slv_req_i.aw.len + 1;
        // AWSIZE > GRANULARITY: clear beat-size / granularity reservations per beat
        if (slv_req_i.aw.size > AXI_ADDR_LSB) begin
            aw_res_len = (slv_req_i.aw.len + 1) << (slv_req_i.aw.size - AXI_ADDR_LSB);
        end
        // AWSIZE < GRANULARITY: clear one reservation for every granularity / beat-size beat
        else if (slv_req_i.aw.size < AXI_ADDR_LSB) begin
            aw_res_len = (slv_req_i.aw.len + 1) >> (AXI_ADDR_LSB - slv_req_i.aw.size);
        end
    end
    ...
    always_comb begin
        ...
        case (aw_state_q)
            AW_IDLE: begin
                if (slv_req_i.aw_valid && !w_cmd_full && b_status_inp_gnt && wifq_inp_gnt) begin
                    // New AW and we are ready to handle it.
                    if (slv_req_i.aw.addr >= ADDR_BEGIN && slv_req_i.aw.addr <= ADDR_END) begin
                        // Inside exclusively-accessible range.
                        // Make sure no exclusive AR to the same address is currently waiting.
                        if (!(slv_req_i.ar_valid && slv_req_i.ar.lock &&
                                slv_req_i.ar.addr[AXI_ADDR_WIDTH-1:AXI_ADDR_LSB] == slv_req_i.aw.addr[AXI_ADDR_WIDTH-1:AXI_ADDR_LSB])) begin
                            // Make sure no exclusive write to the same address is currently in
                            // flight.
                            aw_wifq_exists_inp.data.addr = slv_req_i.aw.addr[AXI_ADDR_WIDTH-1:AXI_ADDR_LSB];
                            aw_wifq_exists_inp.data.excl = 1'b1;
                            // If this is a burst, check the entire page for exclusive writes in flight.
                            if (slv_req_i.aw.len > 0) aw_wifq_exists_inp.mask[12:1] = '0;
                            aw_wifq_exists_req = 1'b1;
                            if (aw_wifq_exists_gnt && !wifq_exists) begin
                                // Check reservation and clear identical addresses.
                                art_check_clr_addr  = slv_req_i.aw.addr[AXI_ADDR_WIDTH-1:AXI_ADDR_LSB];
                                art_check_clr_excl  = slv_req_i.aw.lock;
                                if (mst_aw_ready) begin
                                    art_check_clr_req = 1'b1;
                                end
                                if (art_check_clr_gnt) begin
                                    if (slv_req_i.aw.lock && slv_req_i.aw.len == 8'h00) begin
                                        // Exclusive access and no burst, so check reservation.
                                        if (art_check_res) begin
                                            // Reservation exists, so forward downstream.
                                            mst_aw_valid   = 1'b1;
                                            slv_resp_o.aw_ready = mst_aw_ready;
                                            if (!mst_aw_ready) begin
                                                aw_state_d = AW_WAIT;
                                            end
                                        end else begin
                                        ...
                                        w_cmd_inp = '{forward: art_check_res, id: slv_req_i.aw.id,
                                        ...
                                        if (aw_res_len > 1) begin
                                            ...
                                            clr_addr_d = slv_req_i.aw.addr[AXI_ADDR_WIDTH-1:AXI_ADDR_LSB] + 1;
                                            clr_len_d  = aw_res_len - 1;
                                            clr_id_d   = slv_req_i.aw.id;
                                    ...
                                    b_status_inp_id = slv_req_i.aw.id;
            ...
            AW_BURST: begin
                ...
                // Make sure no exclusive AR to the same address is currently waiting.
                if (!(slv_req_i.ar_valid && slv_req_i.ar.lock &&
                        slv_req_i.ar.addr[AXI_ADDR_WIDTH-1:AXI_ADDR_LSB] == clr_addr_q)) begin
                    // Check reservation and clear identical addresses.
...
// pragma translate_on
    ...
    always_comb begin
        ...
        b_inj_push      = 1'b0;
        w_cmd_pop       = 1'b0;
        if (slv_req_i.w_valid && !w_cmd_empty && !b_inj_full) begin
            ...
            if (slv_resp_o.w_ready && slv_req_i.w.last) begin
                w_cmd_pop = 1'b1;
                if (!w_cmd_oup.forward) begin
...
// pragma translate_on
    ...
    // Control B Channel
    always_comb begin
        slv_b.id            = mst_resp_i.b.id;
        slv_b.resp          = mst_resp_i.b.resp;
        slv_b.user          = mst_resp_i.b.user;
        slv_b_valid         = 1'b0;
        mst_b_ready         = 1'b0;
        b_inj_pop           = 1'b0;
        b_status_oup_id     = '0;
        ...
        case (b_state_q)
            B_NORMAL: begin
                if (!b_inj_empty) begin
                    ...
                    b_status_oup_id = mst_resp_i.b.id;
                    b_status_oup_req = 1'b1;
                    if (b_status_oup_gnt && b_status_oup_valid) begin
                        if (mst_resp_i.b.resp[1] == 1'b0) begin
...
// pragma translate_on
    ...
    assign art_check_id = (aw_state_q == AW_BURST) ?
            clr_id_q
            : AXI_USER_AS_ID ?
            slv_req_i.aw.user[AXI_USER_ID_MSB:AXI_USER_ID_LSB]
            : slv_req_i.aw.id;
    assign art_set_id = AXI_USER_AS_ID ?
            slv_req_i.ar.user[AXI_USER_ID_MSB:AXI_USER_ID_LSB]
            : slv_req_i.ar.id;
    axi_res_tbl #(
